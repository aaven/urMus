...
function StartMovingEvent(r,e)
    ...
    r.touch = "none"
    if #r.list > 0 then
        DPrint(r:Names().." needs to bounce off something")
...
function StartMoving(vv)
    vv:Handle("OnUpdate",nil)
    vv.x,vv.y = vv:Center()
    vv.moving = 1
    vv:Handle("OnUpdate",StartMovingEvent)
end
...
function OKclicked(self)
    ...
    dd.caller.dy = math.tan(d2 * math.pi / 180)
    dd.caller.dx = d1/dd.caller.dy
    dd.caller.dy = d1*dd.caller.dy
    dd.caller.list = dd.obj
    dd.caller.bound = boundary 
    dd.caller.moving = 0
    StartMoving(dd.caller)
    CloseMyDialog()
end
...
function BeBouncedObject(self)
    DPrint("object selected")
    if mydialog.ready == 1 then
        DPrint("inserting "..self:Name())
