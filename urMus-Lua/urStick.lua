...
function MenuUnstick(self)
    ...
    if er ~= -1 then -- it is sticked to other
        ...
        for k,ee in pairs(regions[er].stickee) do
            if ee == self.parent.caller.id then
...
function Menu.Create(region,background,optionList,funcionList,numOptions)
    ...
    menu.num = numOptions
    menu.bkg = background
    menu.position = {"right",0} -- first parameter: left/right. second parameter: offset
...
function UpdateMenuPosition(self)
	...
	if self:Top() > ScreenHeight() then
		self.menu.position[2] = ScreenHeight() - self:Top()
	elseif self:Top() < self.menu.num * self.menu.h then
		self.menu.position[2] = self.menu.num * self.menu.h - self:Top()
	else
